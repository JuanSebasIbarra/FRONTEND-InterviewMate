        ...
        <section className="border-zinc-300 bg-stone-50 p-5 sm:p-7 h-full overflow-y-auto">
          <header className="mb-7 border-b border-zinc-200 pb-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="text-xs uppercase tracking-widest text-zinc-500">{t.history.mainPanel}</p>
          ...
          <article>
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              {!isLoading && !errorMessage && sortedTemplates.length > 0 && (
                <p className="text-sm text-zinc-500">
                  {sortedTemplates.length} {sortedTemplates.length === 1 ? t.history.templateCount : t.history.templatesCount}
                </p>
              )}
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {isLoading && (
                <p className="text-sm text-zinc-600">{t.history.loadingHistory}</p>
              )}
