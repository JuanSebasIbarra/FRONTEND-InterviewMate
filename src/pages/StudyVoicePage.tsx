...
function StudyVoicePage() {
  ...
  useEffect(() => () => { listeningRef.current = false; recognitionRef.current?.abort() }, [])
  ...
  const createAndStart = (lang: string) => {
    ...
    r.onend = () => {
      // Commit any pending interim text so it is not lost
      ...
      // Auto-restart with a FRESH instance (reusing the same instance after onend
      // throws InvalidStateError in Chrome, which was silently swallowed)
    ...
    r.onerror = (event) => {
      if (event.error === 'not-allowed') {
        ...
        setListening(false)
        setError('Permiso de micrófono denegado. Habilítalo e inténtalo de nuevo.')
        return
      }
      // 'no-speech' and 'network' are transient; onend will restart automatically
    }
  ...
  const onToggleMic = () => {
    ...
    createAndStart(selectedLanguage)
  }
