...
export function getQuestionsBySession(sessionId: string) {
  return httpRequest<InterviewQuestion[]>(`/questions/session/${sessionId}`)
}

export function submitAnswer(questionId: string, request: SubmitAnswerRequest) {
  return httpRequest<InterviewQuestion>(`/questions/${questionId}/answer`, {
    method: 'PATCH',
...
export function getQuestionTTS(questionId: string): Promise<Blob> {
  return httpRequestBlob(`/questions/${questionId}/tts`, {
    method: 'GET',
  })
}
