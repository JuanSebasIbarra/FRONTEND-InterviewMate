import type { TranslationKeys } from './es'

export const en: TranslationKeys = {
  // Common
  common: {
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    continue: 'Continue',
    close: 'Close',
    confirm: 'Confirm',
    back: 'Back',
    next: 'Next',
    error: 'Error',
    success: 'Success',
  },

  // Navigation
  nav: {
    dashboard: 'Dashboard',
    history: 'History',
    settings: 'Settings',
    logout: 'Logout',
  },

  // Auth
  auth: {
    login: 'Log in',
    register: 'Sign up',
    username: 'Username',
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm password',
    forgotPassword: 'Forgot password?',
    dontHaveAccount: "Don't have an account?",
    alreadyHaveAccount: 'Already have an account?',
    loginWithGoogle: 'Sign in with Google',
    or: 'or',
  },

  // Dashboard
  dashboard: {
    title: 'Dashboard',
    welcome: 'Welcome',
    recentTemplates: 'Recent templates',
    recentTemplatesSubtitle: 'Showing the three most recently active templates.',
    statistics: 'Statistics',
    statisticsUppercase: 'STATISTICS',
    personalPerformance: 'Personal performance',
    summary: 'Summary',
    summaryDescription: 'Here you have a simple explanation of each statistic to quickly understand your progress.',
    newTemplate: 'New template',
    newTemplateButton: 'New template',
    viewAll: 'View all',
    noTemplates: 'You have no templates registered in your history yet.',
    noTemplatesCreated: 'You haven\'t created any templates yet.',
    loadingTemplates: 'Loading templates...',
    studyCompleted: 'Study completed',
    studySessionsCompleted: 'Completed study sessions',
    studySessionsCompletedDescription: 'Here you can see how many study sessions you have completed from start to finish.',
    interviewsDone: 'Interviews done',
    interviewsDoneDescription: 'This number shows you how many interviews you have already completed on the platform.',
    signInAndPractice: 'Practice days',
    signInAndPracticeLabel: 'Sign-in and practice',
    signInAndPracticeDescription: 'This data reflects the days you logged into your account to practice.',
    defaultStatsDescription: 'This value summarizes your recent activity in this section.',
    barChart: 'Bar',
    pieChart: 'Pie',
    historyUppercase: 'HISTORY',
    additionalTemplatesPlural: 'additional templates',
    additionalTemplatesSingular: 'additional template',
    viewHistory: 'View history',
    lastActivity: 'Last activity',
    open: 'Open',
    new: 'New',
    currentTotal: 'Current total',
    beforeThisMetric: 'Before this metric',
    afterThisMetric: 'After this metric',
    notApplicable: 'Not applicable',
  },

  // Templates
  templates: {
    title: 'Template history',
    subtitle: 'All your templates sorted by recent activity so you can resume any without losing context.',
    position: 'Position',
    enterprise: 'Company',
    lastActivity: 'Last activity',
    createNew: 'Create new template',
    deleteConfirm: 'Delete template?',
    deleteMessage: 'This action will delete {name} from your list. This action cannot be undone.',
    deleting: 'Deleting...',
  },

  // Sessions
  sessions: {
    title: 'Session panel',
    studySession: 'Study session',
    studySessionFull: 'Study Session',
    interview: 'Interview',
    startStudy: 'Study',
    startInterview: 'Interview',
    newSession: 'New session',
    practiceAtYourPace: 'Practice at your pace',
    simulateRealInterview: 'Simulate a real interview',
    status: {
      pending: 'PENDING',
      completed: 'COMPLETED',
      abandoned: 'ABANDONED',
    },
    noStudySessions: 'No study sessions registered for this template.',
    noInterviewSessions: 'No sessions registered for this template.',
    continueSession: 'Continue session',
    deleteSession: 'Delete session',
    sessionCreatedOn: 'Created on',
    whatToDo: 'What would you like to do with this session?',
  },

  // Study
  study: {
    selectTopic: 'Select a study topic',
    topicPlaceholder: 'E.g. Search algorithms',
    generating: 'AI is generating your questions',
    analyzingTopic: 'Analyzing the topic and preparing a personalized session...',
    difficultyAdapted: 'Adapted difficulty',
    uniqueQuestions: 'Unique questions',
    personalizedForYou: 'Personalized for you',
  },

  // Interview
  interview: {
    generating: 'AI is generating your interview questions',
    analyzingTemplate: 'Analyzing the template and preparing personalized questions...',
    contextualQuestions: 'Contextual questions',
    aiEvaluation: 'AI evaluation',
  },

  // Settings
  settings: {
    title: 'Settings',
    subtitle: 'Manage your personal information and security from a single panel',
    personalInfo: 'Personal information',
    personalInfoSubtitle: 'Manage your photo, names and professional description.',
    security: 'Security',
    securitySubtitle: 'Update your username, linked email and password.',
    profilePicture: 'Profile picture',
    firstName: 'First name',
    firstNamePlaceholder: 'E.g. Sebastian',
    lastName: 'Last name',
    lastNamePlaceholder: 'E.g. Ibarra',
    professionalProfile: 'Professional profile',
    professionalProfilePlaceholder: 'Describe your experience, stack and professional goals',
    interviewLanguage: 'Interview language',
    spanish: 'Español',
    english: 'English',
    savePersonalInfo: 'Save personal information',
    saving: 'Saving...',
    personalInfoUpdated: 'Personal information updated.',
    linkedEmail: 'Linked email',
    emailPlaceholder: 'email@example.com',
    newPassword: 'New password',
    newPasswordPlaceholder: 'Leave empty to not change',
    confirmNewPassword: 'Confirm password',
    confirmPasswordPlaceholder: 'Repeat new password',
    saveChanges: 'Save changes',
    securityUpdated: 'Security data updated successfully.',
  },

  // History
  history: {
    title: 'Template history',
    subtitle: 'All your templates sorted by recent activity so you can resume any without losing context.',
    mainPanel: 'Main panel',
    newTemplate: 'New template',
    templateCount: 'template',
    templatesCount: 'templates',
    loadingHistory: 'Loading history...',
    noTemplatesYet: 'You have no templates registered in your history yet.',
    lastActivity: 'Last activity',
    open: 'Open',
  },

  // Template Creation
  templateCreation: {
    title: 'Create new template',
    subtitle: 'Fill in the fields with your voice or keyboard to create a personalized interview template.',
    formLabel: 'FORM',
    backButton: 'Back',
    dictation: 'Dictation',
    dictationHelp: 'Select a field and dictate to fill in',
    browserNotSupported: 'Your browser does not support voice dictation.',
    selectFieldPrompt: 'Select a field to get started.',
    listening: 'Listening',
    dictationApplied: 'Dictation applied to',
    speechError: 'Error processing audio.',
    speechStopped: 'Dictation stopped.',
    speechPaused: 'Dictation paused.',
    browserSpeechNotSupported: 'Your browser does not support speech recognition.',
    startError: 'Could not start voice recognition.',
    startButton: 'Start',
    stopButton: 'Stop',
    activeField: 'ACTIVE FIELD',
    activeFieldHelp: 'This is the field that will be filled when dictating.',
    fieldEnterprise: 'Company',
    fieldType: 'Interview type',
    fieldPosition: 'Position',
    fieldWorkingArea: 'Working area',
    fieldDescription: 'Description',
    fieldRequirements: 'Requirements',
    fieldGoals: 'Goals',
    fieldBusinessContext: 'Business context',
    placeholderEnterprise: 'E.g. Google',
    placeholderPosition: 'E.g. Frontend Developer',
    placeholderWorkingArea: 'E.g. Web development',
    placeholderDescription: 'Describe the position and main responsibilities...',
    placeholderRequirements: 'List the technical requirements and necessary skills...',
    placeholderGoals: 'Define what you expect to achieve in this interview...',
    placeholderBusinessContext: 'Provide context about the company and team...',
    interviewTypeTechnical: 'Technical',
    interviewTypeHR: 'HR',
    interviewTypePsychological: 'Psychological',
    cancelButton: 'Cancel',
    createButton: 'Create template',
    creating: 'Creating...',
    templateCreatedNoId: 'Error: template ID not received.',
    createError: 'Could not create template.',
  },

  // Errors
  errors: {
    loadingProfile: 'Could not load profile.',
    loadingTemplates: 'Could not load templates.',
    loadingSessions: 'Could not load template information.',
    deletingTemplate: 'Could not delete template.',
    deletingSession: 'Could not delete session.',
    aiGenerationFailed: 'AI question generation failed. Please try again later.',
    generic: 'Could not complete the operation. Please try again.',
  },
}
